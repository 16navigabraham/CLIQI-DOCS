...
| **narrativeEngine** | 30m | Fetch emerging trends across all X topics → Grok scoring → Bankr token launch → TG notify premium → post to X (max 3/day) |
| **feeBuyback** | 1h | Claim Bankr creator fees → accumulate per token → buyback at $100 (50% narrative token / 40% CLIQI / 10% Bankr sub) |
